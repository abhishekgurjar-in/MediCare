...
const DoctorAbout = ({ name, about, qualifications, experiences }) => { // Corrected "expereinces" to "experiences"
    ...
    <li key={index} className="flex flex-col sm:flex-row sm:justify-between sm:items-end md:gap-5 mb-[30px]">
      ...
      <div className="mt-4">
        ...
        <ul className="grid sm:grid-cols-2 gap-[30px] pt-4 md:p-5">
          {experiences?.map((item, index) => ( // Corrected "expereinces" to "experiences"
            <li key={index} className="p-4 rounded bg-[#fff9ea]">
              <span className="text-yellowColor text-[15px] leading-6 font-semibold">
                {formatDate(item.startingDate)} -{" "} {/* Corrected the usage of formatDate */}
                {formatDate(item.endingDate)}
              </span>
